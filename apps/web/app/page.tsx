    ...
    <>
      <section className="bg-ink-900 text-white">
        <div className="page-shell grid min-h-[360px] place-items-center py-14 text-center">
          <div>
            <p className="text-sm font-bold uppercase tracking-[0.35em] text-white/70">Buy, Sell and Trade</p>
          ...
          <>
            ...
            <Reveal>
              <section className="grid gap-4 border-y border-ink-200 py-10 md:grid-cols-3">
                {["Lowest asks update from the API", "Authenticated users can watch and sell", "Static pages remain as migration reference"].map((text) => (
                  <div key={text} className="surface p-5">
                    <h3 className="text-lg font-black">{text}</h3>
                    <p className="mt-3 text-sm leading-6 text-ink-500">A sharper marketplace foundation without losing the original storefront rhythm.</p>
                  </div>
                ))}
              </section>
            </Reveal>
